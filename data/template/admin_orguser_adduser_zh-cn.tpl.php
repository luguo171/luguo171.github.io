<?php if(!defined('IN_DZZ')) exit('Access Denied'); /*a:1:{s:85:"C:\Users\Administrator\Desktop\yuanma\dzzoffice\/./admin/orguser/template/adduser.htm";i:1584086883;}*/?>
<div class="main-header" style="padding:0 15px">
  <ul class="nav nav-pills nav-pills-bottomguide">
    <li class="active"> <a href="<?php echo MOD_URL;?>&op=edituser&do=add&orgid=<?php echo $orgid;?>" onclick="jQuery('#orguser_container').load(this.href);return false;">添加用户</a> </li>
  </ul>
</div>
<div class="main-body" style="padding:15px 15px 15px 22px;">
<div id="return_adduser" style="display:none"></div>
<form id="accountform" name="accountform" class="form-horizontal form-horizontal-left" action="<?php echo MOD_URL;?>&op=edituser" method="post" onsubmit="account_submit();return false">
  ...
  <div class="form-group">
    ...
    <div class="controls">
      <input type="text" class="form-control input-sm" id="username" name="username" placeholder="登录用户名" autocomplete="off" value="<?php echo $user['username'];?>" onblur="checknick(this.id);" >
    </div>
    <span id="chk_username" class="help-inline"> <span id="suc_username"></span><kbd class="p_chk"></kbd> 必填，用户在系统中的唯一标识，可用于系统登录 </span> </div>
  <div class="form-group">
    ...
    <span id="chk_phone" class="help-inline">选填，微信绑定的手机号码，员工关注企业号时，会根据员工微信绑定的手机来匹配。</span> </div>
  <div class="form-group">
    ...
    <span id="chk_weixinid" class="help-inline">选填，员工微信号，员工关注企业号时，会根据员工的微信号来匹配。</span> </div>
  <div class="form-group">
    <label class="control-label" for="password">登录密码</label>
    <div class="controls">
      <input type="text" class="form-control input-sm" name="password" autocomplete="off" id="password" placeholder="请填写密码">
    </div>
  ...
  <div class="form-group">
    ...
    <div class="controls">
      <input type="text" class="form-control input-sm" id="password2" name="password2" autocomplete="off" placeholder="请再次输入密码">
    </div>
  ...
  <div class="form-group">
    ...
    <div class="controls ml20"> 
      <!-- <label class="checkbox-inline" ><input type="checkbox" name="adminid" <?php if($_G['adminid']!=1) { ?>disabled="disabled"<?php } ?>value="1"<?php if($user['adminid']==1) { ?>checked<?php } ?>>设为系统管理员
      </label>
      --> <select name="groupid" class="form-control" <?php if($_G[ 'adminid']!=1) { ?>disabled="disabled"<?php } ?>> 
      <?php if(is_array($_G['cache']['usergroups'])) foreach($_G['cache']['usergroups'] as $groupid => $group) { ?> 
      {<?php if(!in_array($groupid ,array('2','3','4','5','6','7','8'))) { ?>
      <option <?php if($groupid==9) { ?>selected="selected"<?php } ?>value="<?php echo $groupid;?>"><?php echo $group['grouptitle'];?>
      </option><?php } ?>
      <?php } ?>
      </select>
    </div>
    ...
    <div id="selorg_container" class="controls selorg-container"> 
      ...
      <ul class="nav nav-pills">
        <li class="dropdown org">
          ...
          <button type="button" id="orgid_<?php echo $org['orgid'];?>_Menu" class="btn btn-default dropdown-toggle" data-toggle="dropdown"> <?php echo $org['depart'];?> <span class="caret"></span> </button>
        ...
        <li class="dropdown job">
          ...
          <ul aria-labelledby="drop-depart" role="menu" class="dropdown-menu">
            ...
            <?php if(is_array($org['jobs'])) foreach($org['jobs'] as $value1) { ?>            <li role="presentation"> <a href="javascript:;" tabindex="-1" role="menuitem" _jobid="<?php echo $value1['jobid'];?>" onclick="selJob(this)"><?php echo $value1['name'];?></a> </li>
            <?php } ?>
          </ul>
        </li>
      ...
      <ul id="nav" class="nav nav-pills">
        <li class="dropdown org">
          ...
          <button type="button" id="orgid_tpml_Menu" class="btn btn-default dropdown-toggle" data-toggle="dropdown"> 请选择机构或部门 <span class="caret"></span> </button>
    ...
    <div id="upjob" class="controls selorg-container">
      <ul class="nav nav-pills">
        <li class="dropdown org">
          ...
          <button type="button" id="uporgid_Menu" class="btn btn-default dropdown-toggle" data-toggle="dropdown"> 请选择机构或部门 <span class="caret"></span> </button>
...
showmessage('抱歉，两次输入的密码不一致', 'danger', 1000, 1);
jQuery('#password').focus();
return false;
}
